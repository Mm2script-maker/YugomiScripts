...
do
	...
	if not Sync then
		error("[Yogomi Spawner] Could not load MM2 Database — use an executor with require() support (e.g. Delta, Xeno, Synapse)")
	end
end
...
pcall(function()
	...
	if tg then
		for _, d in ipairs(tg:GetDescendants()) do
			if d.Name:sub(1, 9) == "ZZVisual_" or d.Name:sub(1, 10) == "AAAYogomi_" then d:Destroy() end
		end
	end
end)
...
---------------------------------------------------------------------
local DATA_URLS = {
	meshes      = "https://raw.githubusercontent.com/Mm2script-maker/mm2-mesh-data/main/mm2_meshes_data.lua",
	richJson    = "https://raw.githubusercontent.com/Mm2script-maker/mm2-mesh-data/main/mm2_rich_cache.json",
	textureCache= "https://raw.githubusercontent.com/Mm2script-maker/mm2-mesh-data/main/mm2_real_texture_cache.json",
}
...
_G.YogomiScanAll = function()
	...
	for i, e in ipairs(SortedList) do
		...
		Hidden[test] = nil
		if i % 25 == 0 then
			print(("[Yogomi Scan] %d/%d..."):format(i, total))
			task.wait(0.05)
		end
	end
	test:Destroy()
	print(("[Yogomi Scan] DONE ok=%d fail=%d generic=%d oversized=%d of %d")
	...
	if next(bad) then
		...
		renderList()
		print(("[Yogomi Scan] pruned %d broken weapons; %d remain"):format(#failIds + #genericList, #SortedList))
	end
...
print(("[Yogomi Spawner] Loaded: %d weapons | dump models: %d (%d complete) | rich captures: %d | flat learned: %d")
